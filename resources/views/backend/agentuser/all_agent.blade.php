                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table">
                    ...
                    @foreach($allagent as $key => $item)
                      <tr>
                        <td>{{ $key+1 }}</td>
                        <td><img src="{{ (!empty($item->photo)) ? url('upload/agent_images/'.$item->photo) : url('upload/no_image.jpg') }}" style="height:40px; width: 70px;"></td>
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->role }}</td>
                        <td>
                        ...
                        <td>
                          <input type="checkbox" data-id="{{ $item->id }}" class="toggle-class" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Active" data-off="Inactive" {{ $item->status ? 'checked' : '' }}>
                        </td>
